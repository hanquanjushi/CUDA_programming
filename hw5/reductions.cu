...
int main(){
  ...
  atomic_red<<<(N+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(d_A, d_sum);
    ...
    printf("atomic_red kernel execution time: %f ms\n", milliseconds);
  ...
  reduce_a<<<blocks, BLOCK_SIZE>>>(d_A, d_sum);
    ...
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&milliseconds, start, stop);
    printf("reduce_a kernel execution time: %f ms\n", milliseconds);
  ...
  reduce_ws<<<blocks, BLOCK_SIZE>>>(d_A, d_sum);
    ...
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&milliseconds, start, stop);
    printf("reduce_ws kernel execution time: %f ms\n", milliseconds);
